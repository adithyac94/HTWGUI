...
public class HtwController implements Features {
  ...
  public HtwController(Readable in, Appendable out) {
    ...
    if (in == null) {
      throw new IllegalArgumentException("The readable object cannot be null");
    }
    if (out == null) {
      throw new IllegalArgumentException("The Appendabel object cannot be null");
    }
  ...
  public void start(Player player) throws IOException {
    ...
    try {
      ...
      if (e.getMessage().equals("Invalid Starting Point")) {
        System.out.println("Invalid starting point...Resetting starting position");
    ...
    if (t) {
      ...
      this.out.append(String.format("You are in Cave " + player.getCurPosition() + "\n"));
   ...
   * The method also adds default if the user provides any invalid input.
   * 
  ...
  public void startGUI(boolean playerMode, String mazeType, int x, int y, int walls, int bats,
    ...
    try {
      ...
      if (e.getMessage().equals("The remaining number of walls are too high")) {
        walls = 0;
      }
      this.player = new PlayerModel(mazeType, x, y, walls, bats, pits, arrows);
  ...
  public void movePlayer(String direction) {
    ...
    view.updatePlayerBatMessage(turn == 1 && playerMode ? 2 : 1, caughtByBat);
    if (moveModel.isEaten()) {
      view.showEatenMessage(turn == 1 && playerMode ? 2 : 1);
    }

    else if (moveModel.hasFallen()) {
  ...
  /**
   * This method lets the user exit from the program.
   * 
   * 
   */
  public void quit() {
